...
%enfatizar integração oauth do sigaa
...
%falar do trabalho na SEDIS com a migração da infra

https://github.com/conda-forge/miniforge/releases/download/26.3.2-0/Miniforge3-26.3.2-0-Linux-x86_64.sh
https://github.com/conda-forge/miniforge/releases/download/26.3.2-0/Mambaforge-26.3.2-0-Linux-x86_64.sh
...
Minha atuação docente no IMD será pautada pela integração entre a fundamentação teórica rigorosa e a experimentação prática intensiva. Acredito que a formação de excelência em Segurança da Informação exige que o aluno não apenas compreenda os protocolos, mas saiba analisar o tráfego de dados em tempo real e antecipar vetores de ataque.

\section{Filosofia de Ensino e Prática Laboratorial}

Pretendo implementar metodologias de \textit{hands-on learning} (aprender fazendo), utilizando laboratórios virtuais e ferramentas de análise de tráfego, como Wireshark, Zeek e Suricata. O objetivo é permitir que os alunos visualizem o fluxo de pacotes e compreendam a anatomia de ataques e defesas diretamente na camada de rede e aplicação.

Além das aulas, planejo desenvolver as seguintes frentes de atuação:

\section{Pesquisa e Inovação em Defesa Cibernética}

Desenvolverei projetos de pesquisa aplicados que visem a proteção do setor produtivo local, especialmente as \textit{startups} do Parque Tecnológico do IMD, focando em:

\begin{itemize}
    \item \textbf{Mitigação de Ataques DDoS}: Pesquisar e testar algoritmos de detecção baseados em inteligência artificial e SDN (\textit{Software Defined Networking}) para identificar e filtrar tráfego malicioso de negação de serviço de forma autônoma na borda da rede.
    \item \textbf{Arquiteturas Anti-Ransomware}: Desenvolver e testar estratégias de defesa em profundidade que combinem análise comportamental de arquivos e sistemas de arquivos com \textit{snapshots} imutáveis, visando a resiliência de dados críticos de empresas juniores e parceiras.
    \item \textbf{Laboratório de Simulação de Ataques (Cyber Range)}: Criar um ambiente controlado para que alunos e empresas possam testar a segurança de seus sistemas contra vulnerabilidades reais em um cenário de \textit{Red Team} vs. \textit{Blue Team}.
\end{itemize}

\section{Projetos de Extensão e Impacto Social}

\textbf{Educação Contra Phishing e Engenharia Social}: Desenvolver campanhas e ferramentas de simulação de \textit{phishing} educacional para a comunidade acadêmica e externa. O foco é capacitar o cidadão comum e funcionários de instituições públicas a identificarem golpes digitais, reduzindo a superfície de ataque humana.

\textbf{Consultoria em Segurança para Startups}: Criar um programa de extensão onde alunos de graduação, sob minha supervisão, realizam auditorias básicas de segurança e sugerem melhorias em infraestruturas de rede para micro e pequenas empresas do Polo Digital, promovendo a cultura de \textit{Security by Design}.

\textbf{Cidadania Digital Segura}: Promover cursos de curta duração voltados à segurança básica de dispositivos móveis e redes domésticas para o público em geral, visando reduzir o impacto de crimes cibernéticos comuns na população de Natal.

\section{Integração Acadêmica e Administrativa}

Espero contribuir ativamente para o fortalecimento das linhas de pesquisa de redes e segurança do IMD, colaborando na atualização curricular das disciplinas de graduação e pós-graduação para que reflitam os desafios contemporâneos da cibersegurança global. Pretendo também atuar em comitês técnicos e na gestão acadêmica, auxiliando no planejamento estratégico de infraestrutura computacional segura do instituto.
